...
Write-Host "`n`n[RUNNING] CalculateDepartmentAllocation for $testsPath" -ForegroundColor Black -BackgroundColor White
...
foreach($test in $tests){
    ...
    $times = Measure-Command { $result = $dept.CalculateDepartmentAllocation() }
    ...
    Write-Host "Node count: $($dept.people.count) ($([int]($dept.people.count/$times.TotalSeconds)) Nodes/Second)"
